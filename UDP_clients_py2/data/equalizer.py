    ...
    def show(self):
        ...
        while run:
            try:
                ...
                # FIXME: someone clever will finalize things
                print(mapped)

                # send pixel colors to frozen bottle
                msgs = ""
                for i in range(MATR_SIZE):
                    for j in range(MATR_SIZE):
                        msgs += str(LedClientBase.rgbF_2_bytes(
                            self.rgb_matr[i][(MATR_SIZE-2)-j])
                        )
                LedClientBase.send(msgs)
